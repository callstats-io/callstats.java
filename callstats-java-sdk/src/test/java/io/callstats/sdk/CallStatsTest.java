...
public class CallStatsTest {
	...
	public void initializeTestWithSendCallStatsEvent() {
		...
		for (int i = 0; i < 2; i++) {
			...
			BridgeStatusInfo bridgeStatusInfo = bridgeStatusInfoBuilder.cpuUsage(randomGenerator.nextInt(100))
					.threadCount(randomGenerator.nextInt(1000)).memoryUsage(randomGenerator.nextInt(5000))
					.intervalRtpFractionLoss(randomGenerator.nextFloat()).avgIntervalJitter(randomGenerator.nextFloat())
					.avgIntervalRtt(randomGenerator.nextFloat()).conferenceCount(randomGenerator.nextInt(100))
					.participantsCount(randomGenerator.nextInt(5000)).videoFabricCount(randomGenerator.nextInt(1000))
					.audioFabricCount(randomGenerator.nextInt(1000)).intervalDownloadBitRate(randomGenerator.nextInt(100))
					.intervalUploadBitRate(randomGenerator.nextInt(100)).totalLoss(randomGenerator.nextInt(100))
					.intervalSentBytes(randomGenerator.nextInt(10000)).intervalReceivedBytes(randomGenerator.nextInt(10000)).build();
	...
	public void statsBuildTest() {
		String res = "{\"type\":\"inbound\",\"ssrc\":\"34567898\",\"packetsSent\":34556,\"bytesSent\":23456,\"rtt\":34,\"avsync\":3.2}";
		ConferenceStats conferenceStats = new ConferenceStatsBuilder().bytesSent(23456).packetsSent(34556).ssrc("34567898").confID("confID")
				.localUserID("2345").remoteUserID("1234").statsType(CallStatsStreamType.INBOUND).rtt(34).avsync(3.2).ucID("ucid").build();
		Gson gson = new Gson();
	...
	public void initializeTestWihSendCallStatsConferenceStartEvent() {
		...
		callstatslib.sendCallStatsConferenceEvent(CallStatsConferenceEvents.CONFERENCE_SETUP, conferenceInfo, new CallStatsStartConferenceListener() {
			...
			public void onResponse(String ucid) {
				...
				ConferenceStats conferenceStats = new ConferenceStatsBuilder().bytesSent(23456).packetsSent(34556).ssrc("34567898").confID(confID)
						.localUserID("2345").remoteUserID("1234").statsType(CallStatsStreamType.INBOUND).jitter(3).rtt(34).avsync(3.2).ucID(ucid)
						.build();
				callstatslib.reportConferenceStats(userID, conferenceStats);

				conferenceStats = new ConferenceStatsBuilder().bytesSent(23456).packetsSent(34556).ssrc("34567899").confID(confID).localUserID("2345")
						.remoteUserID("1234").statsType(CallStatsStreamType.INBOUND).jitter(3).rtt(34).avsync(3.2).ucID(ucid).build();
				callstatslib.reportConferenceStats(userID, conferenceStats);

				conferenceStats = new ConferenceStatsBuilder().bytesSent(23456).packetsSent(34556).ssrc("34567890").confID(confID).fromUserID("2345")
						.localUserID("2345").remoteUserID("1234").statsType(CallStatsStreamType.OUTBOUND).jitter(3).rtt(34).ucID(ucid).build();
				callstatslib.reportConferenceStats(userID, conferenceStats);
